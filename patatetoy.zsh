...
PURE_GIT_UNTRACKED_DIRTY=${PATATETOY_GIT_UNTRACKED_DIRTY}
PURE_CMD_MAX_EXEC_TIME=${PATATETOY_CMD_MAX_EXEC_TIME}
PURE_GIT_DOWN_ARROW=${PATATETOY_GIT_DOWN_ARROW}
PURE_GIT_UP_ARROW=${PATATETOY_GIT_UP_ARROW}
PURE_GIT_PULL=${PATATETOY_GIT_PULL}
PURE_GIT_DELAY_DIRTY_CHECK=${PATATETOY_GIT_DELAY_DIRTY_CHECK:-1800}
VIRTUAL_ENV_DISABLE_PROMPT=12

typeset -A patatetoy_scheme=(
	darkgrey '#6c6d6c'
	red      '#f0522a'
	green    '#82c476'
	yellow   '#fee040'
...
zstyle :prompt:pure:execution_time    color $patatetoy_scheme[yellow]
zstyle :prompt:pure:git:arrow         color $patatetoy_scheme[$PATATETOY_GIT_ARROW_COLOR]
zstyle :prompt:pure:git:branch        color $patatetoy_scheme[$PATATETOY_GIT_BRANCH_COLOR]
zstyle :prompt:pure:git:branch:cached color $patatetoy_scheme[red]
zstyle :prompt:pure:git:dirty         color $patatetoy_scheme[$PATATETOY_GIT_DIRTY_SYMBOL_COLOR]]
zstyle :prompt:pure:git:stash         color $patatetoy_scheme[$PATATETOY_GIT_STASH_SYMBOL_COLOR]]
zstyle :prompt:pure:host              color $patatetoy_scheme[$PATATETOY_USERNAME_COLOR]]
zstyle :prompt:pure:path              color $patatetoy_scheme[$PATATETOY_PATH_COLOR]
zstyle :prompt:pure:prompt:error      color $patatetoy_scheme[$PATATETOY_CURSOR_COLOR_KO]
zstyle :prompt:pure:prompt:success    color $patatetoy_scheme[$PATATETOY_CURSOR_COLOR_OK]
zstyle :prompt:pure:user              color $patatetoy_scheme[$PATATETOY_USERNAME_COLOR]]]
zstyle :prompt:pure:user:root         color $patatetoy_scheme[red]
zstyle :prompt:pure:virtualenv        color $patatetoy_scheme[$PATATETOY_VIRTUALENV_COLOR]]]
...
prompt_pure_human_time_to_var() {
	...
	_patatetoy_cmd_exec_time "${total_seconds}"

	# Store human readable time in a variable as specified by the caller
	typeset -g "${var}"="${patatetoy_human_exec_time}"
}
...
prompt_pure_preprompt_render() {
	...
	if [[ -n $prompt_pure_vcs_info[branch] ]]; then
		...
		local stash="%F{$prompt_pure_colors[git:stash]}"'$prompt_pure_vcs_info[stash]'"%F{$git_color}"
		if [[ -n $prompt_pure_vcs_info[action] ]]; then
			branch+="%F{$prompt_pure_colors[git:action]}"'$prompt_pure_vcs_info[action]'"%F{$git_color}"
		fi
		preprompt_parts+=("$branch""%F{$git_dirty_color}"'${prompt_pure_git_dirty}%f'"${stash}")
	fi
...
prompt_pure_async_vcs_info() {
  ...
  _patatetoy_vcs_info
	...
	local -A info
	info[pwd]=$PWD
	info[top]=$(git rev-parse --show-toplevel)
	info[branch]=$patatetoy_git_branch
	info[action]=$patatetoy_git_action
	info[stash]=$patatetoy_git_stash
...
prompt_pure_check_git_arrows() {
	setopt localoptions noshwordsplit
	_patatetoy_vcs_upstream
	[[ -n $patatetoy_git_upstream ]] || return
	typeset -g REPLY=$patatetoy_git_upstream
}
	...
	case $job in
		...
		prompt_pure_async_git_dirty)
			...
			if (( code == 0 )); then
				...
				typeset -g prompt_pure_git_dirty="${PATATETOY_GIT_DIRTY_SYMBOL}"
			fi
...
prompt_pure_state_setup() {
	...
	[[ -n $ssh_connection || $PATATETOY_FORCE_DISPLAY_USERNAME == 1 ]] \
    && username='%F{$prompt_pure_colors[user]}%n%f'"$hostname"

	# Show `username@host` if root, with username in default color.
	[[ $UID -eq 0 ]] && username='%F{$prompt_pure_colors[user:root]}%n%f'"${$PATATETOY_ROOT_SYMBOL}"
